#!/usr/bin/env python3
"""
parse_tsv_multinode_grid.py

Parse experiments/tsv_multinode/results/*.txt into a CSV summary.
"""

import csv
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent.parent
RESULTS_DIR = ROOT / "experiments" / "tsv_multinode" / "results"
DATA_DIR = ROOT / "experiments" / "tsv_multinode" / "data"
OUT_CSV = DATA_DIR / "tsv_multinode_results.csv"

DATA_DIR.mkdir(parents=True, exist_ok=True)

PATTERNS = {
    "total_area_mm2": re.compile(r"Total Area\s*=\s*([\d.]+)\s*mm\^2"),
    "read_latency_ns": re.compile(r"Cache Hit Latency\s*=\s*([\d.]+)\s*ns"),
    "write_latency_ns": re.compile(r"Cache Write Latency\s*=\s*([\d.]+)\s*ns"),
    "write_energy_nJ": re.compile(r"Cache Write Dynamic Energy\s*=\s*([\d.]+)\s*nJ"),
    "leakage_power_mW": re.compile(r"Cache Total Leakage Power\s*=\s*([\d.]+)\s*mW"),
}

FIELDNAMES = [
    "filename",
    "technology",
    "node_nm",
    "local_tsv_projection",
    "global_tsv_projection",
    "tsv_redundancy",
    "total_area_mm2",
    "read_latency_ns",
    "write_latency_ns",
    "write_energy_nJ",
    "leakage_power_mW",
]


def decode_text(path: Path) -> str:
    for enc in ("utf-16", "utf-8", "latin-1"):
        try:
            return path.read_text(encoding=enc)
        except UnicodeError:
            continue
    return path.read_bytes().decode("latin-1", errors="ignore")


def parse_name(stem: str) -> tuple[int, int, int, float]:
    stem = stem.replace("_output", "")
    match = re.match(r"TSVGrid_3D_SRAM_(\d+)nm_L(\d+)_G(\d+)_R(\d+(?:p\d+)?)$", stem)
    if not match:
        raise ValueError(f"Unrecognized filename format: {stem}")
    node_nm = int(match.group(1))
    local = int(match.group(2))
    global_tsv = int(match.group(3))
    redundancy = float(match.group(4).replace("p", "."))
    return node_nm, local, global_tsv, redundancy


def extract_metrics(text: str) -> dict[str, float | None]:
    metrics = {}
    for key, pattern in PATTERNS.items():
        match = pattern.search(text)
        metrics[key] = float(match.group(1)) if match else None
    return metrics


def main() -> None:
    rows = []
    for path in sorted(RESULTS_DIR.glob("*_output.txt")):
        node_nm, local, global_tsv, redundancy = parse_name(path.stem)
        text = decode_text(path)
        row = {
            "filename": path.name,
            "technology": "3D_SRAM",
            "node_nm": node_nm,
            "local_tsv_projection": local,
            "global_tsv_projection": global_tsv,
            "tsv_redundancy": redundancy,
            **extract_metrics(text),
        }
        rows.append(row)

    with OUT_CSV.open("w", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=FIELDNAMES)
        writer.writeheader()
        writer.writerows(rows)

    print(f"Wrote {len(rows)} rows to {OUT_CSV}")


if __name__ == "__main__":
    main()
